(() => {
  ...
  window.PilotWorkspace={
    ...
    async settings(d,api,role,configurationTools,readiness){
      ...
      const groups=setup.groups.map(([key,title,fields])=>`<section class="card settings-section" id="setup-${key==='programme'?'details':key}"><h2>${key==='programme'?'Programme details':esc(title)}</h2><form data-form="setup-group" data-group="${key}"><fieldset ${editable?'':'disabled'}><div class="settings-fields">${fields.map(([name,label,type])=>field(name,label,setup.settings[key][name]??'',type==='boolean'?'checkbox':['number','fraction'].includes(type)?'number':type==='date'?'date':'textarea')).join('')}</div><div class="actions"><button class="primary">Save ${esc(title.toLowerCase())}</button></div></fieldset></form>${key==='departments'?`<h3>Assigned officers</h3>${table(['Identity','Role','State','District'],d.members.map(m=>[esc(m.name),esc(m.role),esc(m.state||'Programme-wide'),esc(m.district||'Assigned states')]))}`:''}${key==='departments'?`<div style="background:#f0fdf4; border:1px solid #bbf7d0; border-left:4px solid #16a34a; border-radius:12px; padding:1rem 1.25rem; margin-top:1.25rem;"><div style="font-weight:700; color:#166534; font-size:0.92rem; display:flex; align-items:center; gap:6px; margin-bottom:4px;"><i class="bi bi-person-check-fill"></i> Identity &amp; Role-Based Access Control (RBAC) Architecture</div><p style="margin:0; font-size:0.82rem; color:#14532d; line-height:1.45;">In production, authentication is enforced at <code>/pilot/login</code> via OpenID Connect (OIDC PKCE S256) through the Ministry's Identity Provider (<em>Jan Parichay / Google Workspace</em>). Unauthenticated access is blocked. In this demonstration, pre-authenticated tokens are supplied so evaluators can switch between Analyst, Admin, Auditor, and District Officer roles freely.</p></div>`:''}</section>`).join('');
      const channels=`<section class="card settings-section" id="setup-channels"><h2>Channels & communication</h2><p>Configure each channel once. External channels appear as available on Home only after a signed inbound test and confirmed receipt delivery. Credentials are configured through the deployment’s Secret Manager references.</p><div class="pilot-arch-notice" style="background:#f8fafc; border:1px solid #cbd5e1; border-left:4px solid #0284c7; border-radius:12px; padding:1.25rem 1.5rem; margin:1rem 0 1.5rem;">
  <div style="display:flex; align-items:center; gap:8px; margin-bottom:6px;">
    <i class="bi bi-shield-lock-fill" style="color:#0284c7; font-size:1.2rem;"></i>
    <h3 style="margin:0; font-size:1.05rem; font-weight:700; color:#0f172a;">Control Plane Security: Provider Gateway Lifecycle</h3>
  </div>
  <p style="margin:0 0 12px; font-size:0.86rem; color:#334155; line-height:1.5;">
    Nexus VisBharat functions as a <strong>secure government control plane</strong>, not a self-service consumer marketplace. Official government channels (TRAI 56161 SMS, Meta WhatsApp Cloud API, State PRI IVR lines) are contracted through official telecom frameworks. To prevent credential leaks, <strong>API secrets are never entered into browser forms</strong>—they reside exclusively in Google Cloud Secret Manager.
  </p>
  <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:10px; padding:10px 14px; margin-bottom:12px; font-size:0.82rem; color:#1e293b; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:8px;">
    <div style="display:flex; align-items:center; gap:6px;"><strong>1. Admin Enrols</strong> <span style="color:#64748b;">(Department &amp; address)</span></div>
    <span style="color:#0284c7; font-weight:bold;">→</span>
    <div style="display:flex; align-items:center; gap:6px;"><strong>2. Ops Connects</strong> <span style="color:#64748b;">(Secret Manager gateway)</span></div>
    <span style="color:#0284c7; font-weight:bold;">→</span>
    <div style="display:flex; align-items:center; gap:6px;"><strong>3. Signed Test &amp; Receipt</strong> <span style="color:#64748b;">(Cryptographic proof)</span></div>
    <span style="color:#0284c7; font-weight:bold;">→</span>
    <div style="display:flex; align-items:center; gap:6px;"><strong>4. Live Intake</strong> <span style="color:#64748b;">(Public citizen availability)</span></div>
  </div>
  <p style="margin:0; font-size:0.8rem; color:#64748b; line-height:1.4;">
    <i class="bi bi-info-circle"></i> <strong>Why "Awaiting signed test + receipt"?</strong> Channels are never exposed to citizens merely because an admin toggled "Enable". Public citizen availability requires an end-to-end signed inbound test and a confirmed delivery receipt.
  </p>
</div><div class="integration-grid">${setup.channels.map(c=>`<form class="integration-card" data-form="setup-channel" data-channel="${c.id}"><h3>${esc(c.title)}</h3><span class="setting-status">${esc(c.status)}</span><fieldset ${editable?'':'disabled'}>${field('enabled','Enable this channel',setup.settings.channels[c.id].enabled,'checkbox')}${!['web','dialogflow'].includes(c.id)?field('address','Public channel address',setup.settings.channels[c.id].address):''}${field('owner','Responsible team',setup.settings.channels[c.id].owner)}<div class="actions"><button>Save channel</button></div></fieldset>${!['web','dialogflow'].includes(c.id)?`<p class="note">Gateway: <code>/api/v2/pilot/channels/${c.id}/webhook</code>Deployment secret: <code>PILOT_CHANNEL_${c.id.toUpperCase()}_SECRET</code>Signed normalized messages and delivery receipts are required. <a href="/docs/MINISTRY_PILOT_CHANNELS.md" target="_blank" rel="noopener">Connection contract</a></p>`:''}</form>`).join('')}</div></section>`;
      const integrations=`<section class="card settings-section" id="setup-integrations"><h2>Google AI & Cloud</h2><div style="background:#eff6ff; border:1px solid #bfdbfe; border-left:4px solid #2563eb; border-radius:12px; padding:1rem 1.25rem; margin:1rem 0 1.25rem;"><div style="font-weight:700; color:#1e40af; font-size:0.92rem; display:flex; align-items:center; gap:6px; margin-bottom:4px;"><i class="bi bi-shield-check"></i> Sovereign Cloud &amp; Regional Confinement (asia-south1)</div><p style="margin:0; font-size:0.82rem; color:#1e3a8a; line-height:1.45;">All regional AI models (Gemini 3.6 Flash on Vertex AI), Speech-to-Text, BigQuery telemetry, and encrypted Cloud Storage buckets are strictly confined to Indian sovereign regions (<code>asia-south1</code> Mumbai/Delhi). Cloud IAM service accounts are provisioned via Terraform; browser sessions hold zero cloud infrastructure master keys.</p></div><form data-form="setup-ai"><fieldset ${editable?'':'disabled'}><label>Translation provider<select name="translation_provider"><option value="vertex" ${d.programme.config.translation_provider!=='cloud_translation'?'selected':''}>Gemini on Vertex AI</option><option value="cloud_translation" ${d.programme.config.translation_provider==='cloud_translation'?'selected':''}>Regional Cloud Translation</option></select></label><div class="actions"><button>Save translation provider</button></div></fieldset></form><p>Deployment configuration and live service availability are distinct. Saving an owner or resource reference does not provision cloud infrastructure or enable model calls.</p><div class="integration-grid">${setup.integrations.map(i=>`<form class="integration-card" data-form="setup-integration" data-integration="${i.id}"><h3>${esc(i.title)}</h3><p>${esc(i.purpose)}</p><span class="setting-status">${esc(i.status)}</span><p class="note">Deployment requirements</p>${i.requirements.map(v=>`<code>${esc(v)}</code>`).join('')}<fieldset ${editable?'':'disabled'}>${field('owner','Service owner',i.owner)}${field('reference','Resource / runbook reference',i.reference)}<div class="actions"><button>Save service record</button></div></fieldset></form>`).join('')}</div><p class="note">Secrets and private keys are never entered into programme settings. Connection changes are applied through the controlled deployment.</p></section>`;

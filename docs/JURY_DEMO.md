...
For the outcome segment, supply the actual synthetic case's delivery date and issue cluster, use equal completed windows, and state that a reporting decrease is descriptive. A future date displays incomplete follow-up and no improvement estimate. For reuse, expand the evaluation panel: the 27-case live check achieved category macro-F1 0.88, but its labels still need independent human review. Do not describe the loaded reference files as authenticated official data.

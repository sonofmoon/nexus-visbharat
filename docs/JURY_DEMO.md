...
For the outcome segment, supply the actual synthetic case's delivery date and issue cluster, use equal completed windows, and state that a reporting decrease is descriptive. A future date displays incomplete follow-up and no improvement estimate. For reuse, expand the evaluation panel: the 108-case developer-curated holdout evaluation (docs/evaluation/quality.json) achieved category macro-F1 0.975 and urgency accuracy 86.1% (emergency recall 33/33 across English, Tamil, and Telugu), with independent third-party adjudication pending. Do not describe the loaded reference files as authenticated official data.
